<template>
  <view class="chat-container">
    ...
    <view class="top-mode-toggle">
      ...
      <view v-if="!isText2Pic" class="mode-indicator1">
        <tui-tag 
          size="small" 
          padding="6px 10px" 
          :type="isOpenAddtionalFunction ? 'light-blue' : 'gray'" 
          @click="openDrawer"
        >
          <view class="toggle-mode">
            <tui-icon name="tool" :size="16"></tui-icon>
            <text v-if="isOpenAddtionalFunction">附加功能: {{selectedFunction.text}}</text>
            <text v-else>附加功能关闭</text>
          </view>
        </tui-tag>
      </view>
    ...
    <tui-drawer mode="bottom" :visible="visible" @close="closeDrawer">
        ...
        <tui-cascade-selection :reset="reset" height="200px" :itemList="functionList" @complete="complete"></tui-cascade-selection>
    </tui-drawer>
    ...
    >
      ...
      <view v-if="isOpenAddtionalFunction && selectedFunction.text" 
            ...
            :class="{
              ...
              'cooking-tip': selectedFunction.text === '做菜达人',
              'other-tip': !(selectedFunction.text === '中译英' || selectedFunction.text === '英译中') && !selectedFunction.text.includes('评价') && selectedFunction.text !== '做菜达人'
            }">
        <view class="function-tip-content">
          <tui-icon :name="getFunctionIcon(selectedFunction.text)" :color="getFunctionColor(selectedFunction.text)" :size="18"></tui-icon>
          <text class="function-tip-text">
            ...
            <template v-else-if="selectedFunction.text === '中译日'">请输入需要翻译的中文内容</template>
            <template v-else-if="selectedFunction.text === '日译中'">请输入需要翻译的日文内容</template>
            <template v-else-if="selectedFunction.text === '中译法'">请输入需要翻译的中文内容</template>
            <template v-else-if="selectedFunction.text === '法译中'">请输入需要翻译的法文内容</template>
            ...
            <template v-else-if="selectedFunction.text && ['过节', '生日', '祝福', '表白', '分手'].includes(selectedFunction.text)">
              请输入朋友圈文案的关键词或主题，将生成{{selectedFunction.value}}{{selectedFunction.text}}文案
            </template>
            <template v-else-if="selectedFunction.text && ['种草', '吐槽', '分享', '暗广'].includes(selectedFunction.text)">
              请输入小红书文案的关键词或主题，将生成{{selectedFunction.value}}的{{selectedFunction.text}}文案
            </template>
            <template v-else-if="selectedFunction.text && ['衣服', '鞋子', '包包', '化妆品', '数码产品', '闲鱼转转二手'].includes(selectedFunction.text)">
              请输入想要砍价的具体商品，将生成{{selectedFunction.value}}的砍价话术
            </template>
            <template v-else-if="selectedFunction.text === '做菜达人'">
              请输入食材名称，例如：青椒，大蒜，猪肉
            </template>
      ...
      <!-- 图像生成模式提示区域 -->
      <view v-if="isText2Pic && !isOpenAddtionalFunction" class="function-tip-area image-gen-tip">
        <view class="function-tip-content">
          <tui-icon name="picture" color="#9C27B0" :size="18"></tui-icon>
          <text class="function-tip-text">
            请输入详细的图像描述，AI将为您生成相应的图片
          </text>
        </view>
      </view>
  ...
  onLoad() {
    this.functionList = [
      ...
      },
      {
        text: '朋友圈文案',
        children: [
          { text: '过节' ,value:'过节',children:[{text:'二十字',value:'二十字'},{text:'三十字',value:'三十字'},{text:'四十字',value:'四十字'}]},
          { text: '生日' ,value:'生日',children:[{text:'二十字',value:'二十字'},{text:'三十字',value:'三十字'},{text:'四十字',value:'四十字'}]},
          { text: '祝福' ,value:'祝福',children:[{text:'二十字',value:'二十字'},{text:'三十字',value:'三十字'},{text:'四十字',value:'四十字'}]},
          { text: '表白' ,value:'表白',children:[{text:'二十字',value:'二十字'},{text:'三十字',value:'三十字'},{text:'四十字',value:'四十字'}]},
          { text: '分手' ,value:'分手',children:[{text:'二十字',value:'二十字'},{text:'三十字',value:'三十字'},{text:'四十字',value:'四十字'}]},
        ]
      },
      {
        text: '小红书文案',
        children: [
          { text: '种草' ,value:'种草',children:[{text:'五十字',value:'五十字'},{text:'一百字',value:'一百字'},{text:'二百字',value:'二百字'}]},
          { text: '吐槽' ,value:'吐槽',children:[{text:'五十字',value:'五十字'},{text:'一百字',value:'一百字'},{text:'二百字',value:'二百字'}]},
          { text: '分享' ,value:'分享',children:[{text:'五十字',value:'五十字'},{text:'一百字',value:'一百字'},{text:'二百字',value:'二百字'}]},
          { text: '暗广' ,value:'暗广',children:[{text:'五十字',value:'五十字'},{text:'一百字',value:'一百字'},{text:'二百字',value:'二百字'}]},
        ]
      },
      {
        text: '砍价话术',
        children: [
          { text: '衣服' ,value:'衣服',children:[{text:'二十字',value:'二十字'},{text:'三十字',value:'三十字'},{text:'四十字',value:'四十字'}]},
          { text: '鞋子' ,value:'鞋子',children:[{text:'二十字',value:'二十字'},{text:'三十字',value:'三十字'},{text:'四十字',value:'四十字'}]},
          { text: '包包' ,value:'包包',children:[{text:'二十字',value:'二十字'},{text:'三十字',value:'三十字'},{text:'四十字',value:'四十字'}]},
          { text: '化妆品' ,value:'化妆品',children:[{text:'二十字',value:'二十字'},{text:'三十字',value:'三十字'},{text:'四十字',value:'四十字'}]},
          { text: '数码产品' ,value:'数码产品',children:[{text:'二十字',value:'二十字'},{text:'三十字',value:'三十字'},{text:'四十字',value:'四十字'}]},
          { text: '闲鱼转转二手' ,value:'闲鱼转转二手',children:[{text:'二十字',value:'二十字'},{text:'三十字',value:'三十字'},{text:'四十字',value:'四十字'}]},
        ]
      },{
        text: '做菜达人',
      }
  ...
  methods: {
    ...
    getAiResponse(userMessage) {
        ...
        return new Promise((resolve, reject) => {
          ...
          // 如果开启了附加功能，将功能信息加入请求
          if (this.isOpenAddtionalFunction && this.selectedFunction.text) {
            const prefix = this.getFunctionPrefix(this.selectedFunction.text);
            
            // 构建完整的functionType，包含前缀和具体功能
            const fullFunctionType = prefix + this.selectedFunction.text;
            ...
            console.log('附加功能已启用，发送附加数据:', requestData);
          }
    ...
    getFunctionIcon(functionType) {
      if (functionType === '中译英' || functionType === '英译中' || 
          functionType === '中译日' || functionType === '日译中' || 
          functionType === '中译法' || functionType === '法译中') {
        return 'exchange';
      } else if (functionType && (functionType.includes('好评') || functionType.includes('差评'))) {
        return 'star';
      } else if (functionType && ['过节', '生日', '祝福', '表白', '分手'].includes(functionType)) {
        return 'circle-friend';  // 朋友圈图标
      } else if (functionType && ['种草', '吐槽', '分享', '暗广'].includes(functionType)) {
        return 'fabulous';  // 点赞/小红书图标
      } else if (functionType && ['衣服', '鞋子', '包包', '化妆品', '数码产品', '闲鱼转转二手'].includes(functionType)) {
        return 'money';  // 砍价/金钱图标
      } else if (functionType === '做菜达人') {
        return 'shop';  // 使用商店图标表示做菜
      } else {
        return 'tip';
      }
    },
    ...
    getFunctionColor(functionType) {
      if (functionType === '中译英' || functionType === '英译中' || 
        ...
        return '#07C160'; // 绿色 - 朋友圈
      } else if (functionType && ['种草', '吐槽', '分享', '暗广'].includes(functionType)) {
        return '#FF2442'; // 红色 - 小红书
      } else if (functionType && ['衣服', '鞋子', '包包', '化妆品', '数码产品', '闲鱼转转二手'].includes(functionType)) {
        return '#F9A825'; // 黄色 - 砍价
      } else if (functionType === '做菜达人') {
        return '#4CAF50'; // 绿色 - 做菜
    ...
    /**
     * 获取功能前缀
     * 根据功能类型构建适当的前缀，以匹配后端接口
     */
    getFunctionPrefix(functionType) {
      if (['中译英', '英译中', '中译日', '日译中', '中译法', '法译中'].includes(functionType)) {
        return '翻译';
      } else if (['好评', '差评'].includes(functionType)) {
        return '评价';
      } else if (['过节', '生日', '祝福', '表白', '分手'].includes(functionType)) {
        return '朋友圈';
      } else if (['种草', '吐槽', '分享', '暗广'].includes(functionType)) {
        return '小红书';
      } else if (['衣服', '鞋子', '包包', '化妆品', '数码产品', '闲鱼转转二手'].includes(functionType)) {
        return '砍价';
      }
      return '';
    },
...
.image-gen-tip .function-tip-content {
  display: flex;
  align-items: center;
  gap: 6px;
}

.image-gen-tip .function-tip-text {
  font-size: 13px;
  color: #4c5864;
  line-height: 1.5;
  margin-left: 4px;
